  ...
  <div class="header">
	...
	<div class="section">
	  <div class="slider">
	    <div class="container slidercontent">
	      <h3 class="hero" id="hero">게시글 Look</h3>
	    </div>
	  </div>
	</div>
	...
	<div id="editer_container">
		<form name="community_form" action="/boardUpdate_logic" method="POST">
			...
			<tr>
				<th>제목</th>
				<td><input type="text" name="update_title" value="${list.board_title}" disabled></td>
				<input type="hidden" value="${list.board_number}" name="update_number">
			</tr>
			<tr>
				<th class="board_kind">게시글 종류</th>
				<td><input type="text" value="${list.board_kind}" name="update_kind" id="search_kind" style="width: 60px;  text-align: center;" disabled></td>		
			</tr>
			<tr>
				<th class="board_user">작성자</th>
				<td>
					<input type="text" name="update_user" value="${list.board_user}" readonly style="background-color: #e2e2e2; border-style: outset;">
				</td>
			</tr>		
			</table>
				<textarea id="summernote" name="update_detail" onchange="change_go()">${list.board_detail}</textarea>			
			<div style="margin-left: 81%;">    	
				<button type="button" onclick="history.back()" class="form_button" id="update_button">뒤로가기</button>
			</div>
		</form>
	</div>
